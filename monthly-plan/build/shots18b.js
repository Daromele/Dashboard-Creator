...
const APP='file://'+path.resolve('v18.html');
...
(async()=>{
 ...
 await p.evaluate(()=>{state.settings.hiddenNav=[];render();});await p.waitForTimeout(400);
 ...
 const grab=async(name,{scrollTo,ratio=RATIO,offset=0}={})=>{
   ...
   await p.screenshot({path:'deck18/tab-'+name+'.png',clip});
