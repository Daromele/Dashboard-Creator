...
await p.goto('file://'+path.resolve('listing-kit/Monthly_Plan_Etsy_Mockups.html'));await p.waitForTimeout(1800);
const out={};
...
await fc.setFiles(path.resolve('photo.png'));await p.waitForTimeout(600);
out.clickReplaced=(await img.getAttribute('src')).startsWith('data:image/png');
// 2. drop a file onto another picture
const b64=fs.readFileSync(path.resolve('photo.png')).toString('base64');
...
await p.locator('#s2').screenshot({path:'deck18/wide-s2.png'});
