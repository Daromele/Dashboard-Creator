...
- **Home screen**: headline band with the single hero figure (net cash flow) + safe-to-spend/savings-rate/net-worth tiles; four stat tiles with delta + 12-month sparkline; cash flow bars; spending-pace lines (this month's running spend vs last month's); category bars with budget markers (click one to filter Transactions); bills timeline; goal/debt meters; latest activity; household split band.
- **Chart colour is validated, not chosen**: per-theme `chart` arrays and p1/p2/joint come from the dataviz skill validator (`scripts/validate_palette.js`) — run it against the theme's surface before editing any of those hex values. Income is cool, expenses warm, in every theme (the old green/orange pair was ΔE 3.8 under protanopia; p1 vs p2 was ΔE 1.3).
- **Chart plumbing**: `svgSpark`, `hBars` (with `meter: true` for progress), `dailyCumulative`, and one shared crosshair tooltip (`initChartHover`). Container-narrow charts pass a smaller `width` so the viewBox scale stays near 1:1.
- **Responsive**: cards are container-query contexts, so a card reflows from its own width (charts stack + centre under 400px; `.t-stack` tables fold to two lines under 330px). Every bare table is inside `.table-wrap`. `.flex` wraps by default (`.flex.nowrap-row` opts out). The test sweeps all ten views at 1024px and 390px for page/card overflow.
- **Sidebar rail**: `body.rail` (footer button, or `[`) collapses the sidebar to 68px of icons; `ui.rail` is persisted, mobile is unaffected.
- **Household split**: a full-width `.card.inverse` band — two stacked share bars (income, expenses, coloured by person) plus a per-person block (net, share of income, income/expected/expenses). Replaced the six-column table and two donuts, which could not survive a narrow column.
...
7. "Redo the home screen — bold, eye-catching, wow": new Overview (hero band, stat tiles with sparklines and deltas, cash flow + spending-pace charts, category bars, bills timeline, goal/debt meters, latest activity), a validated chart palette across all six themes, a shared chart tooltip, and a fix for line charts rendering invisible under prefers-reduced-motion.
