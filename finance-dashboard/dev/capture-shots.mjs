...
async function setup(ctx) {
  ...
  await page.waitForTimeout(300); await page.click('#tourSkip'); await page.waitForTimeout(300); await page.evaluate(() => document.getElementById('toasts').innerHTML = '');
...
await page.click('#nav button[data-view=overview]'); await page.waitForTimeout(100);
for (const t of ['cream', 'charcoal', 'midnight', 'sage', 'blush', 'slate']) { await page.click(`#themeDots [data-theme=${t}]`); await page.waitForTimeout(250); await page.evaluate(() => document.getElementById('toasts').innerHTML = ''); await page.screenshot({ path: path.join(out, `theme-${t}.jpg`), type: 'jpeg', quality: 80, clip: { x: 0, y: 0, width: 1440, height: 900 } }); }
await page.click('#themeDots [data-theme=cream]'); await page.waitForTimeout(200);
...
// tablet
const tctx = await browser.newContext({ viewport: { width: 1024, height: 768 }, deviceScaleFactor: 1.5, reducedMotion: 'reduce' });
